...
const NOTE_BY_ID = {
  ...
  pike_pushup: 'הגרסה הביתית של לחיצת כתפיים. ישבן גבוה, ראש יורד קדימה בין הידיים.',
  ...
  towel_row_iso: 'מושכים במגבת נגד יד שנייה — התנגדות בלי משקל, טובה כשאין כלום.',
  doorframe_towel_row: 'חתירה ביתית: משקל הגוף נגד משקוף. ככל שהרגליים קדימה יותר, קשה יותר.',
  table_row: 'חתירה מתחת לשולחן יציב — הדרך הזולה ביותר למשוך משקל גוף בבית.',
  ...
  band_bent_over_row: 'חתירה בכפיפה עם גומייה — מתאים כשאין משקולות בבית.',
  ...
  band_lat_pulldown: 'מורידים את המרפקים לצלעות. הגומייה מדמה פולי עליון בבית.',
  ...
  tempo_squat: 'שלוש שניות למטה. האטה היא הדרך להוסיף עומס כשאין משקולות.',
...
function layout(profile, vol, budget, split) {
  ...
  for (const group of priority) {
    ...
    for (let k = 0; k < want; k++) {
      ...
      const pattern = pats[taken % pats.length];
      d.picks.push({
        group,
        pattern,
        occurrence: d.picks.filter((s) => s.pattern === pattern).length,
        order: fallbackPattern ? 99 : d.def.patterns.indexOf(pattern),
        seq: taken,
      });
    }
  }
