...
const GOAL_BASE = {
  ...
  /*
   * Cut to match the intensity, on a three-consultant consensus.
   *
   * Every rep-based hypertrophy set is now taken to failure or within two reps
   * of it. All three advisors independently returned push 12 / pull 12 /
   * legs 12 / core 6 / calves 4, and the honest reason is fatigue and time
   * rather than stimulus overshoot: the failure-vs-non-failure meta-analyses
   * (Grgic 2022, Refalo 2023) find the hypertrophy difference trivial-to-small,
   * while the recovery cost of a set to failure is well established at 24-48h
   * against 6-24h. So the stimulus per set went up modestly and the bill went up
   * a lot, and 16 / 1.3 is 12.
   *
   * arms 8 -> 5 is the largest single cut and it is not about RIR at all.
   * Counting primary AND secondary exposure across a generated week, the
   * triceps already receive 18 sets and the shoulders 21 against the chest's 9 —
   * in a plan whose largest allocated group is "push". Direct arm work is being
   * added on top of a bill the compounds have already paid.
   *
   * Not 4: at 4 the small groups fall under the survivors() bar and vanish
...
export function sessionBudget(profile) {
  ...
  /*
   * Hypertrophy loses a slot for the same reason strength has always lost one.
   *
   * distributeSets() floors every exercise at 2, so delivered volume can never
   * fall below 2x the exercise count and cutting the target alone does nothing
   * once you are near that floor. And a 7-movement upper day where every set
   * goes to failure is 7 cold-start fatigue spikes on a 9-minute warm-up
   * budget — the exercise count drives overuse harder than the set count does.
   */
  const goalAdj = (goal === 'strength' || goal === 'muscle') ? -1 : (goal === 'fatloss' ? 1 : 0);
...
export function weeklyVolume(profile) {
  ...
  if (timeBound && sum > 0) {
     ...
     */
    /*
     * The recovery term used to appear here too, and it cancelled itself out.
     *
     * `sum` is already proportional to recovery — every group is
     * GOAL_BASE[g] * f.total, and f.total contains f.years — so multiplying the
     * numerator by the same factor divided straight back out. Measured: k came
     * to 1.164 at ages 25, 55, 68 and 80 alike, and a 25-year-old and an
     * 80-year-old at 5x75 received push 18 / pull 18 / legs 18, identically.
     * The age curve in this file asked for a 14% reduction and delivered 4%.
     *
     * The fill target drops from 0.85 to 0.70 for the same reason the volume
     * did. The comment above already said extra time is worth more as rest than
     * as sets; at RIR 0-2 that is emphatically true, and 0.85 contradicted it.
     */
    const k = Math.min(1.15, (capacity * 0.70) / Math.max(1, sum));
    if (k > 1) {
