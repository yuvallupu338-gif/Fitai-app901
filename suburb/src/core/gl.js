 ...
 *
 * Not a framework. It creates the context, compiles programs and introspects
 * their uniforms once so call sites can say `p.u.uSunDir` instead of caching
 * locations by hand, and it builds the three kinds of object the renderer
 * needs: array textures, render targets, and vertex arrays.
 *
 * WebGL2 is required and not polyfilled. The whole look depends on texture
 * arrays (every material of the neighbourhood in one bind), float render
 * targets (the bloom on a sodium lamp) and non-power-of-two mipmapping, and a
 * WebGL1 path would cost all three.
 */
...
export function createContext(canvas) {
  const gl = canvas.getContext('webgl2', {
     ...
     * trade worth making for a game meant to be played on a phone in bed. */
  });
  if (!gl) return null;
...
/*
 * One 2D array texture holding every material in the neighbourhood, one per
 * layer. This is why the whole street draws in a handful of calls: the
 * material index rides along in the vertex data, so lawn, siding, roof
 * shingle, road and window all come out of one buffer with one bind.
 */
 ...
 * seam, one fewer vertex, and the fragment shader gets its UV from
 * gl_VertexID, so there is no vertex buffer to bind at all.
 */
...
/*
 * The vertex layout, described once and shared by the mesh builder and the
 * renderer so the two cannot drift apart.
 *
 * `aFlick` is in the list and that is not an accident. A vertex writer that
 * emits one more float than the layout describes does not fail loudly: the
 * extra float lands on the next vertex's first component and is immediately
 * overwritten, the attribute is never enabled, and the shader silently reads
 * the WebGL default of 0 for every vertex in the world. The symptom is that
 * every flickering fixture in the game flickers in perfect unison, which looks
 * like a stylistic choice rather than a bug. The sibling app in `backrooms/`
 * has exactly that mismatch today. Sizes here are in floats; if you add a
...
export const VERTEX_LAYOUT = [
  ...
  { name: 'aAO',    size: 1 },
  { name: 'aMat',   size: 1 },
  { name: 'aFlick', size: 1 },   /* per-fixture phase, or -1 for "steady" */
];
