...
if (existsSync(html)) {
  ...
  /*
   * The on-device model needed 'wasm-unsafe-eval' and a worker. It was removed
   * after it locked up a machine, and its permissions were removed with it —
   * which is the thing this checks. A grant taken for a feature has to leave
   * when the feature does, or the policy becomes a list of everything this page
   * was ever allowed to do.
   */
  check(!/unsafe-eval/.test(policy),
    'html: no eval grant of any kind — the feature that needed one is gone');
  check(/worker-src 'none'/.test(policy),
    'html: no workers — the feature that needed one is gone');
  check(!/hf\.co|huggingface|githubusercontent/.test(policy),
    'html: no model-weight hosts — the feature that needed them is gone');
