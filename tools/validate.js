...
/*
 * Every filmed demonstration must point at an exercise that exists.
 *
 * demos.js is generated by mapping another app's catalogue onto these ids, so
 * its keys are the one part of the data nothing else validates: a renamed or
 * deleted exercise leaves a demo stranded, and a stranded demo is invisible —
 * it simply never shows, and the card silently falls back to the rig. Several
 * megabytes of payload would ship for a card no one can reach.
 */
{
  try {
    const demos = await load('src/data/demos.js');
    const idx = await load('src/data/exercises.index.js');
    const orphans = Object.keys(demos.DEMOS || {}).filter((id) => !idx.byId(id));
    for (const id of orphans.slice(0, 8)) {
      err(`demos.js has a film for "${id}", which is not an exercise id — `
        + `it will never render and its payload ships for nothing`);
    }
    if (orphans.length > 8) err(`...and ${orphans.length - 8} more orphaned films`);
