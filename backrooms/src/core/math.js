/*
 * math.js — the small pile of linear algebra this renderer actually uses.
 *
 * Everything is Float32Array and column-major, which is what WebGL wants, so
 * matrices go to uniformMatrix4fv with transpose=false and no copying.
 *
 * There is no generic vector type. Positions are plain {x,y,z} objects where
 * they are long-lived and readable (player, entities), and raw triples where
 * they are hot (mesher inner loops). Mixing the two on purpose is cheaper than
 * allocating a vector class per frame.
 */
