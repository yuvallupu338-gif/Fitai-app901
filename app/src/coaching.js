...
function algChips(inputId,current){var cur=(current||'').toLowerCase();return '<div class="pills" style="margin-top:8px">'+ALLERGY_CHIPS.map(function(o){var on=cur.indexOf(o[1].toLowerCase())>=0;return '<button type="button" class="pill'+(on?' act':'')+'" data-call="algToggle" data-args="'+esc(JSON.stringify([inputId,o[1]]))+'">'+o[0]+'</button>';}).join('')+'</div>';}
function algToggle(inputId,word){var btn=this;var inp=document.getElementById(inputId);if(!inp)return;var parts=inp.value.split(/[,\u060c;\n]/).map(function(x){return x.trim();}).filter(Boolean);var lw=word.toLowerCase();var idx=-1;for(var k=0;k<parts.length;k++){if(parts[k].toLowerCase()===lw){idx=k;break;}}if(idx>=0){parts.splice(idx,1);btn.classList.remove('act');}else{parts.push(word);btn.classList.add('act');}inp.value=parts.join(', ');try{inp.dispatchEvent(new Event('input',{bubbles:true}));}catch(e){}try{inp.dispatchEvent(new Event('change',{bubbles:true}));}catch(e){}}
